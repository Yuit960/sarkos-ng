...
uint8_t ustack_t1[STACK_SIZE] __attribute__((aligned(4096), section(".user_data")));
uint8_t ustack_t2[STACK_SIZE] __attribute__((aligned(4096), section(".user_data")));
...
void init_pagination()
{
  ...
  //KERNEL and USER sections mapped separetly to avoid USER memory accessing KERNEL memory

  //KERNEL Mapping
  for (uint32_t addr = 0; addr < 0x400000; addr += 0x1000)
  {
    map_page(pgd_t1, addr, addr, PG_KRN | PG_RW);
    map_page(pgd_t2, addr, addr, PG_KRN | PG_RW);
  }
...
{
  asm volatile("leave ; pusha      \n"
               "testb $3, 36(%esp) \n" //Testing if we interrupted kernel or user code
               "jz .skip_save      \n"
               "movl %esp, tmp_esp \n"
               ".skip_save:        \n"
               "call schedule      \n"
...
void __regparm__(1) syscall_handler(int_ctx_t *ctx)
{
  ...
  default:
    debug("Cannot handle the interruption number %d\n", int_n);
    break;
  }
}
...
void schedule()
{
  if (current_task != -1) tasks[current_task].kstack_top = tmp_esp;
...
__attribute__((section(".user_text"))) void sys_counter(uint32_t *counter)
{
  asm volatile("int $0x80" : : "a"(1), "b"(counter) : "memory");
}

__attribute__((section(".user_text"))) void user1(void)
{
...
__attribute__((section(".user_text"))) void user2(void)
{
...
void tp()
{
  ...
  //Debug infos
  debug_infos(gdtr, pgd_t1, pgd_t2, user1, ustack_t1, user2, ustack_t2);

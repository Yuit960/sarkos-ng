...
void tp() {
   ...
   while((uint32_t)entry < (info->mbi->mmap_addr + info->mbi->mmap_length)) {
      ...
      debug("[0x%16llx - 0x%16llx] - ", entry->addr, entry->addr+entry->len -1);
   ...
   //Test available and reserved memory
   debug("\nTesting available and reserved memory:\n");
   ...
   *ptr_in_reserved_mem = 0xaaaaaaaa;                           // write
   ...
   int *outptr = (int*)0x8000000;
   debug("Value of memory at 128MB: %x\n", *outptr);
   *outptr = 0xcafe;
   debug("New value of memory at 128MB: %x\n", *outptr);
}
